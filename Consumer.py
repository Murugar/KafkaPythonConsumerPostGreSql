...
class PyConsumer(Client):
    ...
    def createDBConn (self):
        try:
            self.conn = psycopg2.connect("dbname= 'flowresult' user= 'postgres' host='localhost' password='postgres'")
    ...
    def do_commit (self, immediate=False, async=None):
        ...
        self.dbg('Committing Kafka to PostgresqlDB  %d messages (Async=%s)' %
    ...
    def msg_consume (self, msg):
        ...
        if msg.error():
            if msg.error().code() == KafkaError._PARTITION_EOF:
                ...
                self.err('Kafka Consume failed: %s' % msg.error(), term=True)
            return
...
if __name__ == '__main__':
    ...
    try:
        ...
        pass
    pc.dbg('Closing Kafka Consumer ' + str(datetime.now()))
